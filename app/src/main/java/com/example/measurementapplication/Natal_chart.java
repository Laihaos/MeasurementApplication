...
//                textyan.append("\n"+EarthlyBrancheslist.get(i));
        ...
        for (int i = 0; i < ziweiBrancheslist.size(); i++) {
            ...
            if (i == ziWeiStarPosition) {

                System.out.println("紫微星的位置在：" + ziweiBrancheslist.get(i - 1) + "宫");
//                紫薇
                ziwei=ziweiBrancheslist.get(i - 1);
...
//        System.out.println("十四主星" + XingyaoBrancheslist);
        ...
        for (int i = 0; i < zuofu.length; i++) {
            if (i == Month) {
                Xingyao(0, ziweiBrancheslist, XingyaoBrancheslist, zuofu[i-1],TextViewlist,"左辅");
            }
            if (i == Month) {
                Xingyao(0, ziweiBrancheslist, XingyaoBrancheslist, list.get(i-1),TextViewlist,"右弼");
            }
        }


        for (int i = 0; i < EarthlyBranches.length; i++) {
            if (birthday.equals(list.get(i))) {
                Xingyao(0, ziweiBrancheslist, XingyaoBrancheslist, EarthlyBranches[i],TextViewlist,"文昌");
            }
            if (birthday.equals(zuofu[i])) {
                Xingyao(0, ziweiBrancheslist, XingyaoBrancheslist, EarthlyBranches[i],TextViewlist,"文曲");
            }
...
//    四化
        ...
        for (int i = 0; i < 4; i++) {
            si= sihuamap.get(tian).substring(i,i+1);
            for (int j = 0; j < TextViewlist.size(); j++) {
                TextView a =TextViewlist.get(j);

//                if (a.getText().equals(si)||si.equals()) {
//                    Log.d( "onCreate: ", String.valueOf(a));
//                    a.append("\n"+);
//                }
            }
